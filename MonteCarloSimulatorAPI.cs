...
namespace MonteCarloSimulatorAPI.Models
{
    ...
    public class StockPriceGenerator  // Created a new class just in case user is curious about the stock paths and want to graph it
    {
        ...
        public (double[] path, double CV) GenerateGBMPath(double S0, double mu, double K, double sigma, double T, int steps, double[] normalRandoms, SimulationParams p, bool isCall)
        {
            ...
            if (double.IsNaN(path[^1]) || double.IsInfinity(path[^1]))
            {
                Console.WriteLine($"❌ Stock Price NaN at T+0.1 - Last Path Value: {path[^1]}, Steps: {p.Steps}, T: {T}");
            }
    ...
    public class SimulationParams {
        ...
        // Barrier-related properties
        public bool IsBarrierOption { get; set; } // Flag to indicate a barrier option
        public double BarrierLevel { get; set; } // Barrier level (if applicable)
        public BarrierType BarrierType { get; set; } // Barrier type (DownAndOut, UpAndOut, etc.)
   ...
   public class YieldCurve {
    ...
    public double GetTenorRate(double T) {
        return _rate; // Just return the passed-in rate
    }
}
    ...
    public static class Simulator
    {
        ...
        public static EvaluationResult Evaluate(Option o, YieldCurve c, SimulationParams p, StockPriceGenerator generator)
        {
            ...
            Console.WriteLine($"📌 Monte Carlo Simulation - T: {T}, T+0.1: {T + 0.1}");
            ...
            if (p.IsParallel)
            {
                Parallel.For(0, p.Simulations, i =>
                {
                    ...
                    AddPayoffs(o, paths, pathsAntithetic, K, T, r, payoffBags, CVs, CV2s,p);
                });
            }
            else
            {
        ...
        private static (double[][] paths, double[] CVs) GeneratePaths(double S0, double r, double K, double sigma, double T, SimulationParams p, StockPriceGenerator generator, double[] normalRandoms, bool isCall)
        {
            ...
            if (paths[5].Any(double.IsNaN))
            {
                Console.WriteLine($"❌ Stock Price Path NaN Detected at T+0.1");
            }
        ...
        private static bool IsBarrierBreached(double[] path, SimulationParams p)
        {
            if (!p.IsBarrierOption) return false; // Not a barrier option
            ...
            {
                case BarrierType.DownAndOut:
                    return minPrice < p.BarrierLevel;
                case BarrierType.UpAndOut:
                    return maxPrice > p.BarrierLevel;
                case BarrierType.DownAndIn:
                    return minPrice < p.BarrierLevel;
                case BarrierType.UpAndIn:
                    return maxPrice > p.BarrierLevel;
        ...
        private static void AddPayoffs(Option o, double[][] paths, double[][] antitheticPaths, double K, double T, double r, ConcurrentBag<double>[] payoffBags, double[] CVs, double[] CV2s,SimulationParams p)
        {
            double beta1 = -1;
            ...
            for (int j = 0; j < paths.Length; j++)
            {
                ...
                if (IsBarrierBreached(paths[j], p))
                {
                    payoff = 0; // Skip payoff and CV contributions if barrier is breached
                }

                else if (antitheticPaths != null && CVs != null)
                {
    ...
    class Program
    {
         static void Main(string[] args)
        {
            ...
            YieldCurve c = new YieldCurve(0.05);
            ...
            switch (optionChoice)
            {
                ...
                case 3: // Barrier Option
                    ...
                    double barrierLevel = double.Parse(Console.ReadLine());
                    ((BarrierOption)option).Barrier = barrierLevel;
                    p.BarrierLevel = barrierLevel; // Update SimulationParams
                    ...
                    p.BarrierType = (BarrierType)(barrierTypeChoice - 1); // Update SimulationParams

                    Console.WriteLine("Is it a Call Option? (true/false):");
                    bool isCall = bool.Parse(Console.ReadLine());
                    ((BarrierOption)option).IsCall = isCall;

                    // Mark as a barrier option in SimulationParams
                    p.IsBarrierOption = true;
                    break;
                case 4:
                    // Lookback Option
